...
# Adding lagged returns and moving average
return$lag1 <- lag(return$return, 1)
return$lag2 <- lag(return$return, 2)
return$sma5 <- rollmean(return$return, 5, fill = NA, align = 'right')

# Scaling features
scaled_data <- as.data.frame(scale(return[, c("lag1", "lag2", "sma5")], center = TRUE, scale = TRUE))
colnames(scaled_data) <- paste(colnames(scaled_data), "scaled", sep = "_")
return <- cbind(return, scaled_data)

# Splitting data into training and testing sets
train_indices <- 1:(n * 0.8)
test_indices <- (n * 0.8 + 1):n

# Creating a validation subset from the test indices
validation_indices <- sample(test_indices, length(test_indices)/2)
validation_data <- return[validation_indices, ]
...
dtrain <- lgb.Dataset(data = as.matrix(train_data[, -which(names(train_data) == "return")]), label = train_data$return)
dtest <- lgb.Dataset(data = as.matrix(test_data[, -which(names(test_data) == "return")]), label = test_data$return)
...
final_model <- lgb.train(
  params,
  dtrain,
  nrounds = 100,  # Set a reasonable static number of rounds
  valids = list(test = dtest),
  early_stopping_rounds = 10
)

# Predicting and calculating errors
predictions <- predict(final_model, newdata = as.matrix(test_data[, -ncol(test_data)]))
mae <- mean(abs(predictions - test_data$return))
rmse <- sqrt(mean((predictions - test_data$return)^2))
cat("Optimized MAE:", mae, "\n")
cat("Optimized RMSE:", rmse, "\n")
```
I need to recheck this ⬆️.
