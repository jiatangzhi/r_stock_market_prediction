...
- We removed the columns **"ghg_s1"**, **"ghg_s2"**, **"ghg_s3"**, because it is irrelevant.
...
The only categorical value is "ticker" and we will analyse the 5 top ones.
...
```{r}
# Using the first 5 tickers to analyse
top_tickers_data <- head(aggregated_data, 20)
...
* Pie charts are not ideal for datasets with many categories because they can become cluttered and difficult to interpret, so we use only the top 20 tickers for simplicity. 
...
We obtained a really good accuracy for the predictions, with a **MAE** of *0.082*, & **RMSE** of *0.126*.
